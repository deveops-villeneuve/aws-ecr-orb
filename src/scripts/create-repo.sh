...
if [ "$PARAM_PUBLIC_REGISTRY" == "1" ]; then
    aws ecr-public describe-repositories --profile "${PARAM_PROFILE_NAME}" --region "${REGION}"--repository-names "${REPO}" >/dev/null 2>&1 ||
        aws ecr-public create-repository --profile "${PARAM_PROFILE_NAME}" --region "${REGION}" --repository-name "${REPO}"
else
